...
```sh
git clone https://github.com/side-xp/node-facile.git
cd node-facile
npm install
```
...
- **`npm run coverage`**: Run tests with code coverage output
- **`npm run lint`**: Check issues in code style
- **`npm run format`**: Check issues in code style and fix them when possible
...
1. Make sure you are using a recent version of *Node* (we don't support versions prior to v16)
...
Please refer to our [general Coding Style specification](https://github.com/side-xp/docs/blob/main/coding-style/node-package.md) to learn more about the conventions used in this project.
...
Releases are fully automated via [`semantic-release`](https://semantic-release.gitbook.io) and triggered by merging to `master` branch. You do not need to update the version or changelog manually.
